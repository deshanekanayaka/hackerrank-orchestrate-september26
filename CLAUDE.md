...
8. **Commit after every completed loop.** Message names the stage. Commit on a branch named `stage/<N>-<short-slug>`, then merge it into `main` locally with `git merge --no-ff` and push `main` to `origin`. GitHub same-fork PRs fail for this fork network, so do not attempt them. Never push directly to the upstream contest repo.
