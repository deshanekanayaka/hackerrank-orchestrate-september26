...
8. **Commit after every completed loop.** Message names the stage. Commit on a branch named `stage/<N>-<short-slug>`. Then stop and ask me to approve the merge before running `git merge --no-ff` and `git push origin main`. GitHub same-fork PRs fail for this fork network, so do not attempt them. Never push directly to the upstream contest repo.
